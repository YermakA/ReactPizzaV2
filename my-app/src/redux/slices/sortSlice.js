...
const initialState = {
  ...
  sortTypeArr: [{ name: 'популярности', typeProperty: 'rating' }, { name: 'цене', typeProperty: 'price' }, { name: 'алфавиту', typeProperty: 'title' }],
    ...
    reducers: {
      ...
      getCurrentType(state, action) {
        state.sortTypeArr.filter(type => {
          if (type.typeProperty === action.payload.sortProperty) state.currentType = type
        })
      }
    }
  }
